...
P2020 = {
    'CCSR': {
        ...
        #              Interleaved SDRAM mappings are instead mapped sequentially.
        'OBJECT': '.soc.ccsr',
        'TYPE': 'qoriq-p2-ccsr',
        'registers': {
            'LAWAR': {
    ...
    'CPU': {
        ...
        'registers': {
            ...
            # extra performance monitor counter registers (not found in P2020 docs)
            'pmc4': {},
            'pmc5': {},
            'pmc6': {},
            'pmc7': {},
    ...
    'DMA': {
        # LIMITATIONS: The DMA pause feature and DMA halt features have not been implemented.
        'count': 2,
        'OBJECT': '.soc.dma',
        'TYPE': 'qoriq-p2-dma',
        'registers': {
    ...
    'ECM': {
        # LIMITATIONS: Handles enabling/disabling the cores only. Access errors are not monitored nor reported.
        'OBJECT': '.soc.ecm',
        'TYPE': 'qoriq-p2-ecm',
        'registers': {
            'EEBPCR': {}
    ...
    'ESDHC': {
        # LIMITATIONS: PIO mode not implemented (i.e. DMA must be used). Command and Data CRC check is not supported.
    ...
    'ESPI': {
        ...
        #              Transmission speed settings are unimplemented. When the throttle attribute is set,
        #              3.125 Mbit is used as an upper bound for the bitrate.
        'OBJECT': '.soc.espi',
        'TYPE': 'qoriq-p2-espi',
        'registers': {
            'regs_CS_SPMODE': {
    ...
    'ETSEC': {
        # LIMITATIONS: Huge frames, interrupt coalescing, statistics counters,
        #              lossless flow control and timer related functions are not implemented.
        #              Only RXF interrupt is generated when receiving buffers.
        'count': 3,
        'OBJECT': '.soc.etsec',
        'TYPE': 'qoriq-p2-etsec',
        'registers': {
            ...
            'DMACTRL': {
                ...
                'fields': {
                    ...
                    'GRS': (31-27, 31-27),
                    'GTS': (31-28, 31-28),  # Tx is not restarted when cleared
                    # Not Implemented
                    # 'TOD': (31-29, 31-29),
                    # 'WWR': (31-30, 31-30),
                    'WOP': (31-31, 31-31)
            ...
            # 'RXIC': {
            #     'fields': {
            #         # Field ICCS - Coalescing on timer is not yet supported.
        ...
        'registers': {
            'L2CTL': {
                'fields': {
                    ...
                    'L2SIZ': (31-3, 31-2),
                    'Reserved5': (31-8, 31-4),
                    'L2DO': (31-9, 31-9),        # Write-access not implemented
                    'L2IO': (31-10, 31-10),      # Write-access not implemented
                    'Reserved4': (31-11, 31-11),
                    'L2INTDIS': (31-12, 31-12),  # Write-access not implemented
                    'L2SRAM': (31-15, 31-13),
                    'Reserved3': (31-17, 31-16),
                    'L2LO': (31-18, 31-18),      # Write-access not implemented
                    'L2LSC': (31-19, 31-19),     # Write-access not implemented
                    'Reserved2': (31-20, 31-20),
                    'L2LFR': (31-21, 31-21),
                    'L2LFRID': (31-23, 31-22),   # Write-access not implemented
    ...
    'PCIE': {
        # LIMITATIONS: PCI capabilities (i.e. the standard PCI capabilities defined in a linked list
        #              in PCI configuration space) are typically only dummy registers, as PCI capabilities are usually
        #              too low-level for Simics. Error management registers only have dummy implementation.
        'count': 3,
        ...
        'registers': {
            ...
            'PEX_inbound_IWAR': {
                ...
                'fields': {
                    'EN': (31-0, 31-0),
                    'Reserved2': (31-1, 31-1),
                    'PF': (31-2, 31-2),        # Not forced to 0 when TRGT = CCSRBAR
                    'Reserved1': (31-7, 31-3),
                    'TRGT': (31-11, 31-8),
                    'RTT': (31-15, 31-12),     # Only read transaction type implemented (0x4)
                    'WTT': (31-19, 31-16),     # Only write transaction type implemented (0x4)
    ...
    'PIC': {
        # LIMITATIONS: The priority between interrupts with the same xVPR[PRIORITY] values are not guaranteed.
    ...
    'RAPIDIO': {
        # Limitations:
        # These parts of the RapidIO controller is implemented:
        #       Maintenance operations: read/write of configuration registers.
    ...
    'USB': {
        ...
        #              Cannot handle when USB devices send NAK or STALL status during Control and Bulk transfers
        #              Cannot handle USB devices with a latency(submit_transfer must return USB_Transfer_ Completed)
        #              Bulk and Control transfers will only be initiated while the attribute
        #              async_list_polling_enabled is set.
        #              Asynchronous schedule park capability not implemented.
        ...
        'registers': {
            ...
            'usb_regs_frindex': {},  # The controller may process frames in advance
            'usb_regs_hccparams': {},
            'usb_regs_hciversion': {
                'bits': 16,
            },
...
for target in P2020:
    ...
    for register in P2020[target]['registers']:
        if 'bits' in P2020[target]['registers'][register]:
            ...
            bits = 32
        if 'count' in P2020[target]['registers'][register]:
            count = 1
            for dimension in P2020[target]['registers'][register]['count']:
                count *= dimension
        else:
            count = 1
